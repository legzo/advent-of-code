...
import gg.jte.aoc.v2018.GuardEvent.FallingAsleepEvent
import gg.jte.aoc.v2018.GuardEvent.NewShiftEvent
import gg.jte.aoc.v2018.GuardEvent.WakingUpEvent
...
class Day04Test {
    ...
    fun `parse new shift event`() {
        ...
        when (val event = GuardEvent.fromString("[1518-11-01 00:00] Guard #10 begins shift")) {
            is NewShiftEvent -> {
    ...
    fun `parse new shift event and correct date for early start`() {
        ...
        when (val event = GuardEvent.fromString("[1518-11-01 23:58] Guard #99 begins shift")) {
            is NewShiftEvent -> {
    ...
    fun `parse falling asleep event`() {
        ...
        when (val event = GuardEvent.fromString("[1518-11-01 00:05] falls asleep")) {
    ...
    fun `parse waking up event`() {
        ...
        when (val event = GuardEvent.fromString("[1518-11-01 00:25] wakes up")) {
            is WakingUpEvent -> {
                event.timeStamp.day shouldBe LocalDate.of(1518, 11, 1)
